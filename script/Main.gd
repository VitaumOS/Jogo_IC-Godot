...
# ----------------------------------------------------------
# LÓGICA DE SALVAMENTO DE PADRÕES
# ----------------------------------------------------------
...
func _on_padraocorte_salvo_pressed():
	...
	# 1. Cria o dicionário de dados do padrão
	var dados_padrao = {
	...
	for peca in pecas_encaixadas:
		dados_padrao.pecas.append({
			"largura_peca": peca.largura_peca, # <--- ACESSO CORRIGIDO
			"caminho_textura": peca.caminho_textura # <--- ACESSO CORRIGIDO
		})

	# 2. Salva na lista
	padroes_corte_salvos.append(dados_padrao)
	
	# 3. Exibe o novo padrão
...
func _exibir_padrao_salvo(dados: Dictionary):
	...
	var altura_visualizacao: float = 50.0 # Altura fixa desejada para o display
	# 4. Cria visualmente cada peça
	for dados_peca in dados.pecas:
		# --- NOVO: Envolve o Sprite2D em um nó Control para gerenciar o layout ---
		var wrapper_control = Control.new()
		var peca_visual = Sprite2D.new()
		
		var textura_carregada = load(dados_peca.caminho_textura)
		peca_visual.texture = textura_carregada
		if textura_carregada:
			var largura_escalada = dados_peca.largura_peca 
			wrapper_control.custom_minimum_size = Vector2(largura_escalada, altura_visualizacao)
			peca_visual.position = Vector2(largura_escalada / 2.0, altura_visualizacao / 2.0)	
		wrapper_control.add_child(peca_visual)
		visualizador_container.add_child(wrapper_control)
	# 5. Adiciona o nó de exibição ao contêiner principal
...
# ----------------------------------------------------------
# LÓGICA DE UI E CRIAÇÃO DE PEÇAS
# ----------------------------------------------------------
...
func _on_peca_disponivel_clicada(indice: int):
	...
	# 3. Conecta o sinal de remoção
	instancia_peca.peca_removida.connect(_on_peca_removida_por_clique) # <--- SINAL CORRIGIDO

	# 4. Adiciona ao pai das peças
	pai_pecas.add_child(instancia_peca)
	
	# 5. Define a posição inicial no estoque
...
func _on_peca_removida_por_clique(script_peca: PieceScript):
	if is_instance_valid(script_peca):
		if script_peca.esta_no_container: # <--- ACESSO CORRIGIDO
			_tentar_encaixar_peca(script_peca)
		_organizar_pecas_estoque()
		_atualizar_texto_resultado("Peça descartada ou removida do container.")

# ----------------------------------------------------------
# LÓGICA DE ENCAIXE E POSICIONAMENTO
# ----------------------------------------------------------

func _tentar_encaixar_peca(peca: PieceScript) -> bool:
	var largura_restante = largura_container - largura_total_atual
	
	# 1. PEÇA CLICADA PARA SAIR (Retorno ao Estoque)
	if peca.esta_no_container: # <--- ACESSO CORRIGIDO
		pecas_encaixadas.remove_at(pecas_encaixadas.find(peca))
		largura_total_atual -= peca.largura_peca # <--- ACESSO CORRIGIDO
		peca.esta_no_container = false # <--- ACESSO CORRIGIDO
		_reposicionar_pecas_encaixadas()
		_organizar_pecas_estoque()
		_atualizar_texto_resultado("Peça removida do container.")
		return true

	# 2. PEÇA SENDO INSERIDA NO CONTAINER
	if peca.largura_peca <= largura_restante: # <--- ACESSO CORRIGIDO
		peca.esta_no_container = true # <--- ACESSO CORRIGIDO
		...
		peca.position = peca.posicao_original # <--- ACESSO CORRIGIDO
		return false
...
func _reposicionar_pecas_encaixadas():
	...
	for p in pecas_encaixadas:	
		...
		x_atual += p.largura_peca # <--- ACESSO CORRIGIDO
...
func _organizar_pecas_estoque():
	...
	for child in pai_pecas.get_children():
		if child is PieceScript and not child.esta_no_container: # <--- ACESSO CORRIGIDO
			child.position = Vector2(retangulo_container.global_position.x + x_deslocamento, y_estoque)
			child.posicao_original = child.position # <--- ACESSO CORRIGIDO
			x_deslocamento += child.largura_peca + 10 # <--- ACESSO CORRIGIDO
