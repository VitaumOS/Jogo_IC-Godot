...
func _salvar_quantidades_atuais():
	...
	var dict_qtds = Global.get_meta("quantidades_padroes_salvas")
	...
	for linha in itens_da_lista:
		if linha.has_meta("composicao"):
			var comp = linha.get_meta("composicao")
			dict_qtds[str(comp)] = linha.get_quantidade()
...
func _conectar_sinais_botoes_quantidade():
	...
	for linha in vbox_padroes_lista.get_children():
		...
		var nó_perda = linha.find_child("Perda")
		if nó_perda:
			nó_perda.visible = (Global.dia_atual != 1)
...
func _exibir_padrao_na_lista(item: Dictionary):
	...
	if Global.has_meta("quantidades_padroes_salvas"):
		var dict_qtds = Global.get_meta("quantidades_padroes_salvas")
		var chave = str(item.composicao)
		if dict_qtds.has(chave):
			var qtd_salva = dict_qtds[chave]
			if "quantidade" in linha:
				linha.quantidade = qtd_salva
			if linha.has_method("set_quantidade"):
				linha.set_quantidade(qtd_salva)
			elif linha.has_method("definir_quantidade"):
				linha.definir_quantidade(qtd_salva)
